...
class ExportCommand(val filePath: String, val phoneBook: MutableMap<String, Person>) : Command {
    override fun isValid(): Boolean {
        return true
    }
    ...
    fun exportToJson() {
        val jsonData = buildString {
            append("[")
            phoneBook.values.forEachIndexed { index, person ->
...
fun main(){
    ...
    var command: Command?
    val commands = mutableListOf<Command>() // Создаем пустой список команд
    do {
        print("Введите команду: ")
        command = readComand(phoneBook, commands)
...
fun Person.toJson(): String {
    return buildString {
        ...
        append("\"name\":\"$name\",")
        append("\"phones\":[")
        append(phone.joinToString { "\"$it\"" })
        append("],")
        append("\"emails\":[")
        append(email.joinToString { "\"$it\"" })
        append("]")
        append("}")
    }
}
...
fun readComand(phoneBook: MutableMap<String, Person>, commands: MutableList<Command>, input: String? = null): Command? {
    ...
    return when (parts[0]) {
        ...
        "export" -> parseExportCommand(parts, phoneBook)
        else -> null
    }
}

fun parseExportCommand(parts: List<String>, phoneBook: MutableMap<String, Person>): Command? {
    if (parts.size != 1) return null
    val filePath = parts[0]
    return ExportCommand(filePath, phoneBook)
}
